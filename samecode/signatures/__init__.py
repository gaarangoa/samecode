...
def median_aggregation_df(data, signatures, signature_column='signature', gene_column='gene_id', median_scaling=True):
    values = []
    names = []
    columns = set(list(data))
    sgs = signatures.drop_duplicates(signature_column)[signature_column]
    for signature in sgs:
        
        genes = signatures[signatures[signature_column] == signature][gene_column]
        # check genes are in data
        
        if len(set(genes) - columns) > 0:
            print("These genes are not present in the data and are ignored: {}: {}".format(signature, set(genes) - columns))
        
        genes = set(genes) & columns
        value = data[genes].median(axis=1)
        if median_scaling:
